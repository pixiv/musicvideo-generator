...
module.exports = class extends Synchronic {
  ...
  update(time) {
    ...
    const pn = simplexNoise(
      movement.wet * 0.08 + time * 0.02,
      movement.wet * 0.08 + time * 0.02,
      time * 0.1
    );
    ...
    this._container.scale.x = 1 + (movement.wet * 0.01);
    this._container.scale.y = 1 + (movement.wet * 0.01);

    this._container.x = this._renderer.width  / 2 + noiseCoordinate[0] * (movement.wet * 2);
    this._container.y = this._renderer.height / 2 + noiseCoordinate[1] * (movement.wet * 2);


    // RGB分離とブラー
    this._filters.rgbShift.uniforms.red = [
      noiseCoordinate[0] * (movement.wet * 2),
      noiseCoordinate[1] * (movement.wet * 2),
    ];
    ...
    this._filters.rgbShift.uniforms.blink = blink.wet * 0.1 + 1;

    this._filters.dirBlur.uniforms.blurStrength = [
      noiseCoordinate[0] * movement.wet,
      noiseCoordinate[1] * -movement.wet,
    ];
  }

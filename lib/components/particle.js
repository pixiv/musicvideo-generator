...
class Particles extends DisplaySynchronic {
  ...
  update(time) {
    ...
    for (let count = 0; count < (wet * 0.1) + 1; count++) {
    ...
    for (const sprite of this.displayObject.children) {
      ...
      if (age < sprite.lifetime) {
        ...
        const scale = (1 + wet * 10) * this._scale;

        sprite.x += Math.sin(rad) * (wet * 0.5 + 1) + Math.cos(Math.PI * 2 * particleNoise) * (0.2 + wet * 0.6);
        sprite.y += Math.cos(rad) * (wet * 0.5 + 1) + Math.sin(Math.PI * 2 * particleNoise) * (0.2 + wet * 0.6);

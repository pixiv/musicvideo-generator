...
module.exports = class extends DisplaySynchronic {
  ...
  constructor(params, audioAnalyser, renderer) {
    ...
    // それぞれの帯域は byteFrequencyData ではどこからどこまで？
  ...
  update() {
    ...
    // 素のままの byteFrequencyData は
    // [低音,   中音, 中音,   高音, 高音, 高音, 高音, 高音, 高音]
    // のような偏りをしているので、
    // これを
    // [低音, 低音, 低音,   中音, 中音, 中音,   高音, 高音, 高音]
    // な感じに rescale する
    const rescaled = Array(this._params.barCount); // rescale 後の byteFrequencyData
    ...
    for (let bar = 0; bar < this._params.barCount; bar++) {
      ...
      let volume = this._audioAnalyser.byteFrequencyData[index];

      // 以下は低音ごまかしキット。
      // 低音はあまり解像度よく取れない（byteFrequencyData の要素数が barUnit 未満しかない）ため、こんなものが必要。
      if (bandIndexRange.top <= 300) {
        const lostRange = barUnit / bandIndexRange.top; // 何倍くらい要素数が足りないか
        const nextVolume  = this._audioAnalyser.byteFrequencyData[index + 1];
      ...
      rescaled[bar] = biasVolume(Math.max(Math.pow(volume * 0.006, 8) * this._params.barTall, 1));
    }
    ...
    case 0: // 直線カラム
      ...
      for (let bar = 0; bar < this._params.barCount; bar++) {
        this.displayObject.drawRect(
          (bar * margin) + (margin / 2),
          this._renderer.height - rescaled[bar] * this._params.barTall,
          this._params.width, rescaled[bar] * this._params.barTall);
      }
      break;
    ...
    case 1: // 円カラム
      ...
      for (let bar = 0; bar < this._params.barCount; bar++) {
        ...
        this.displayObject.moveTo(  // 内側に向かって伸びる
          this._renderer.width  / 2 + Math.sin(angle) * (this._params.radius - rescaled[index] * 0.5),
          this._renderer.height / 2 + Math.cos(angle) * (this._params.radius - rescaled[index] * 0.5));
        this.displayObject.lineTo(  // 外側に向かって伸びる
          this._renderer.width  / 2 + Math.sin(angle) * (this._params.radius + rescaled[index] * 3),
          this._renderer.height / 2 + Math.cos(angle) * (this._params.radius + rescaled[index] * 3));
      }
      break;
    ...
    case 2: // 円連続
      ...
      for (let bar = 0; bar < this._params.barCount; bar++) {
        ...
        this.displayObject[bar === 0 ? 'moveTo' : 'lineTo'](
          this._renderer.width  / 2 + Math.sin(angle) * (this._params.radius + rescaled[index] * 2),
          this._renderer.height / 2 + Math.cos(angle) * (this._params.radius + rescaled[index] * 2));
      }
      break;
    ...
    case 3: // 直線塗りつぶし
      ...
      this.displayObject.moveTo(
        0,
        this._renderer.height - rescaled[0] * this._params.barTall);
      ...
      for (let bar = 1; bar < this._params.barCount - 2; bar++) {
        this.displayObject.quadraticCurveTo(
          ...
          this._renderer.height - rescaled[bar + 1] * this._params.barTall);
      }

...
module.exports = class extends DisplaySynchronic {
  ...
  constructor(video, params, renderer) {
  ...
  initialize(time) {
    ...
    if (source.HAVE_ENOUGH_DATA === source.readyState) {
      coverDisplayObject(this.displayObject, this._renderer.width, this._renderer.height);
      promises.push(new Promise(resolve => source.addEventListener('seeked', resolve)));
    } else {
      promises.push(new Promise(resolve => source.addEventListener('canplaythrough', () => {
        coverDisplayObject(this.displayObject, this._renderer.width, this._renderer.height);
        resolve();
      }, { once: true })));
